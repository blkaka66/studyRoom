...
//    EnterHistoryEntity findActiveByCustomerId(@Param("customerId") Long customerId);//쿼리조건 추가할거면 이름에 active라고 추가해야함
    ...
    // 특정 회원의 현재 활성화된 입장 기록을 조회하는 메서드
    // 조건: customerId가 일치하고, expiredTime이 현재 시간 이전이며, closeTime이 null인 레코드
    @Query("SELECT e FROM EnterHistoryEntity e WHERE e.member = :member AND e.exitTime IS NULL")
    EnterHistoryEntity findActiveByMember(@Param("member") MemberEntity member);//쿼리조건 추가할거면 이름에 active라고 추가해야함
    ...
    int countActiveEntriesByShop(@Param("shop") ShopEntity shop, @Param("currentTime") OffsetDateTime currentTime);


    // 하루에 이용한 사용자 수(memberid로 구분)
    @Query("SELECT COUNT(DISTINCT e.member.id) FROM EnterHistoryEntity e WHERE e.shop = :shop AND e.enterTime BETWEEN :startOfDay AND :endOfDay")
    int countUniqueUsersByShopAndDate(@Param("shop") ShopEntity shop,
...
//    List<Long> findActiveOccupiedSeatIdsByShop(@Param("shopId") Long shopId);
    ...
    List<EnterHistoryEntity> findByShop(ShopEntity shop);
